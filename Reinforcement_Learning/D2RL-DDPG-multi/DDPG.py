...
class DDPG(object):
    def __init__(self, num_inputs, action_space,n, args):
        ...
        self.critic = QNetwork(num_inputs, action_space.shape[0], args.hidden_size,n).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=args.lr)
        self.critic_target = QNetwork(num_inputs, action_space.shape[0], args.hidden_size,n).to(self.device)
        hard_update(self.critic_target, self.critic)

        self.policy=Policy(num_inputs,action_space.shape[0],args.hidden_size,n,action_space).to(self.device)
        self.policy_optim=Adam(self.policy.parameters(),lr=args.lr)
        self.policy_target=Policy(num_inputs,action_space.shape[0],args.hidden_size,n,action_space).to(self.device)
